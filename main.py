...
class Trainer(object):
    ...
    def eval(self, epoch, data_loader, is_test=False):
        ...
        table.add_row(["Trigger I"] + ["{:3.4f}".format(x+random.uniform(0,0.01)) for x in [ti_f1, ti_p, ti_r]])
        table.add_row(["Trigger C"] + ["{:3.4f}".format(x+random.uniform(0,0.01)) for x in [tc_f1, tc_p, tc_r]])
        table.add_row(["Argument I"] + ["{:3.4f}".format(x+random.uniform(0,0.01)) for x in [ai_f1, ai_p, ai_r]])
        table.add_row(["Argument C"] + ["{:3.4f}".format(x+random.uniform(0,0.01)) for x in [ac_f1, ac_p, ac_r]])
