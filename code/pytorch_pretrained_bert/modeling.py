...
class BertForTriggerClassification(BertPreTrainedModel):
    """BERT model for token-level classification.
    This module is composed of the BERT model with a linear layer on top of
    the full hidden state of the last layer.
...
class BertLSTMForTriggerClassification(BertPreTrainedModel):
    """BERT LSTM model for token-level classification.
    """
    def __init__(self, config, num_labels):
...
class BertForQuestionAnswering_withIfTriggerEmbedding(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction). with if_trigger_embedding
    """
    def __init__(self, config):
        ...
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.selfAttention = SelfAttention(num_attention_heads=8, input_size=768, hidden_size=768,
                                           dropout=0.1)
        self.bert_base = self.selfAttention(config.hidden_size + config.hidden_size // 10, 2)
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, if_trigger_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        ...
        # import ipdb; ipdb.set_trace()
        logits = self.bert_base(torch.cat([sequence_output, if_trigger_embedding_output], dim=2))
        start_logits, end_logits = logits.split(1, dim=-1)
        start_logits = start_logits.squeeze(-1)
        end_logits = end_logits.squeeze(-1)
